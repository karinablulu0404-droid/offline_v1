...
spec = importlib.util.spec_from_file_location(
    "public_func",
    r"D:\zg6\offline_v1\stream-py\public_func.py"
)
public_func = importlib.util.module_from_spec(spec)
spec.loader.exec_module(public_func)
import public_func
...
db_config = {
    ...
    'port': int(properties.get("mysql.port", 3306)),  # ✅ 转 int，给默认值
...
class SpiderCHNAmapWeatherData:
    def __init__(self, refresh_interval=500, city_result=None, process_id=None):
        ...
        if city_result is not None:
            ...
            print("【调试】execute_sql 返回：", self.city_result)
        self.logger = logger

...
from (select id,
      ...
      where dt = '20250919') province
         left join
     (select id,
             region_name
      from ods_base_region_full
      where dt = '20250919') region
...
-- 把 ds 和 id 都放在 SELECT 最后一列
INSERT OVERWRITE TABLE dim_user_zip PARTITION (ds, id)
SELECT
    concat(substr(data.name, 1, 1), '*') AS name,
    if(data.phone_num regexp '^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\\d{8}$',
       concat(substr(data.phone_num, 1, 3), '****', substr(data.phone_num, 8, 4)), null) AS phone_num,
    if(data.email regexp '^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\\.[a-zA-Z0-9_-]+)+$',
       concat('*@', split(data.email, '@')[1]), null) AS email,
    data.user_level,
    data.birthday,
    data.gender,
    data.create_time,
    data.operate_time,
    '20250919' AS start_date,
